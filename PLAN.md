...
- [ ] Палитра меток workspace в настройках.
- [ ] Привязка меток к задаче.
- [ ] Фильтры на доске: по метке / приоритету / исполнителю / поиск по тексту (FTS5).

### Этап 7. Полировка (½ дня)
- [ ] Empty states, скелетоны при загрузке.
- [ ] Hotkeys: `?` — список шорткатов, `c` — новая задача, `g p` — к проектам.
- [ ] Toaster для успеха/ошибок.
- [ ] Адаптив до 768px (доска становится скроллящейся, sidebar collapsable).
- [ ] Проверить prefers-reduced-motion и контрастность.
